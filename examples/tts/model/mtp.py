...
class Qwen2MTPAudioDecoder(nn.Module):
    def __init__(self, qwen_config, hidden_size, audio_vocab_size, mtp_num):
        super().__init__()
        ...
        for layer_idx in range(self.mtp_num):
            mtp_layer = nn.ModuleDict(
                {
                    "decoder_layer": Qwen2DecoderLayer(qwen_config, layer_idx=layer_idx),
    ...
    @staticmethod
    def _build_audio_supervision_mask(audio_labels):
        if audio_labels is None:
            return None
        if audio_labels.dim() != 3:
            raise ValueError(f"audio_labels must have shape [B, code_layer, T], got {tuple(audio_labels.shape)}")
        return audio_labels.ne(-100).any(dim=1)
        # if audio_labels.shape[1] < self.mtp_num:
        #     raise ValueError(
        #         f"audio_labels must provide at least {self.mtp_num} lower audio layers for MTP conditioning, "
        #         f"got shape {tuple(audio_labels.shape)}."
        #     )

        # conditioning_tokens = []
        # batch_size = audio_labels.shape[0]
        # for layer_idx in range(self.mtp_num):
        #     safe_tokens = self._sanitize_audio_tokens(audio_labels[:, layer_idx, :])
        ...
        #     )
        #     conditioning_tokens.append(torch.cat([safe_tokens[:, 1:], pad_column], dim=1))
    ...
    #         raise ValueError("conditioning_audio_tokens must contain at least one lower audio layer.")
    ...
    def _build_full_causal_mask(hidden_states, attention_mask=None):
        ...
        mtp_attention_mask = torch.full(
            (batch_size, 1, seq_len, seq_len),
            fill_value=min_dtype,
            dtype=dtype,
            device=device,
        )
        mtp_attention_mask = mtp_attention_mask.masked_fill(allowed.unsqueeze(1), 0)
        return mtp_attention_mask
    def _decode(self, hidden_states, mtp_attention_mask, position_ids):
    ...
    #     #         mtp_caches.append(DynamicCache.from_legacy_cache(None))
    #     #     return mtp_caches

    #     device = prefix_hidden_states.device
    #     attention_mask = torch.ones((batch_size, seq_len), dtype=torch.long, device=device)
    #     mtp_attention_mask = self._build_full_causal_mask(prefix_hidden_states, attention_mask)
    ...
    #     mtp_caches = []
    #     for mtp_layer in self.mtp_layers:
    #         mtp_cache = DynamicCache.from_legacy_cache(None)
    #         conditioned_hidden_states = current_hidden_states + self.audio_embedding(pad_tokens)
    #         current_hidden_states = mtp_layer["decoder_layer"](
    #             conditioned_hidden_states,
    #             attention_mask=mtp_attention_mask,
    #             position_ids=position_ids,
    #             past_key_value=mtp_cache,
    #             output_attentions=False,
    #             use_cache=True,
    #         )[0]
    #         mtp_caches.append(mtp_cache)
    #     return mtp_caches

    # def infer_one_layer_step(self, hidden_states, conditioning_audio_token, layer_idx, mtp_cache):
    #     if hidden_states.dim() != 3 or hidden_states.shape[1] != 1:
    #         raise ValueError(
    #             "hidden_states must have shape [B, 1, H] for incremental MTP inference, "
    ...
    #         raise ValueError("mtp_cache must not be None for incremental MTP inference.")

    #     safe_conditioning_token = self._sanitize_audio_tokens(conditioning_audio_token)
    #     if safe_conditioning_token.shape[1] != 1:
    #         raise ValueError(
    #             "conditioning_audio_token must have shape [B, 1] during incremental MTP inference, "
    #             f"got {tuple(safe_conditioning_token.shape)}."
    #         )

    #     conditioned_hidden_states = hidden_states + self.audio_embedding(safe_conditioning_token)
    #     past_length = mtp_cache.get_seq_length()
    ...
    def forward(self, hidden_states, attention_mask, audio_labels):
        supervision_mask = self._build_audio_supervision_mask(audio_labels)
        if supervision_mask is not None and supervision_mask.shape != hidden_states.shape[:2]:
            raise ValueError(
                "audio_labels supervision mask must match hidden state sequence length "
                f"(got {tuple(supervision_mask.shape)} vs {tuple(hidden_states.shape[:2])})."
            )
        ...
        mtp_attention_mask = self._build_full_causal_mask(hidden_states, attention_mask)
        position_ids = self._build_position_ids(attention_mask)
        # return self._decode(hidden_states, mtp_attention_mask, position_ids, conditioning_audio_tokens)
        return self._decode(hidden_states, mtp_attention_mask, position_ids)

    def infer(self, hidden_states):
        if hidden_states.dim() != 3:
            raise ValueError(f"hidden_states must have shape [B, T, H], got {tuple(hidden_states.shape)}")

        batch_size, seq_len = hidden_states.shape[:2]
        attention_mask = torch.ones((batch_size, seq_len), dtype=torch.long, device=hidden_states.device)
        mtp_attention_mask = self._build_full_causal_mask(hidden_states, attention_mask)
        position_ids = self._build_position_ids(attention_mask)
        return self._decode(hidden_states, mtp_attention_mask, position_ids)
        # conditioning_audio_tokens = self._validate_conditioning_tokens(hidden_states, conditioning_audio_tokens)
